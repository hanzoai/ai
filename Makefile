...
# /server and its ENTRYPOINT, released as `aid` by goreleaser. cmd/routerdoc,
# cmd/check_ddl and cmd/pg2sqlite are developer tools nothing deploys, so this
# does not link them; `go build ./...` still compiles them and `make lint` still
# vets them.
