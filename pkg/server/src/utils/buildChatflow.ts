...
import { IFileUpload, convertSpeechToText, ICommonObject, addSingleFileToStorage, addArrayFilesToStorage } from '@hanzo/ai-components'
...
import { IAction } from '@hanzo/ai-components'
...
export const utilBuildChatflow = async (req: Request, socketIO?: Server, isInternal: boolean = false): Promise<any> => {
    try {
        ...
        if (isFlowReusable()) {
            ...
            if (!nodeToExecute) {
                throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Node not found`)
            }
        ...
        if (e instanceof InternalHanzoError && e.statusCode === StatusCodes.UNAUTHORIZED) {
            throw e
        } else {
