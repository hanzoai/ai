...
export default class Start extends Command {
    ...
    static flags = {
        ...
        HANZO_FILE_SIZE_LIMIT: Flags.string(),
        PORT: Flags.string(),
        ...
        HANZO_SECRETKEY_OVERWRITE: Flags.string(),
        ...
        DISABLE_HANZO_TELEMETRY: Flags.string(),
    ...
    async stopProcess() {
        logger.info('Shutting down Hanzo...')
        try {
            // Shut down the app after timeout if it ever stuck removing pools
            setTimeout(() => {
                logger.info('Hanzo was forced to shut down after 30 secs')
            ...
            logger.error('There was an error shutting down Hanzo...', error)
        }
        process.exit(processExitCode)
    }
    ...
    async run(): Promise<void> {
        ...
        // Authorization
        if (flags.HANZO_USERNAME) process.env.HANZO_USERNAME = flags.HANZO_USERNAME
        if (flags.HANZO_PASSWORD) process.env.HANZO_PASSWORD = flags.HANZO_PASSWORD
        if (flags.APIKEY_PATH) process.env.APIKEY_PATH = flags.APIKEY_PATH

        // API Configuration
        if (flags.HANZO_FILE_SIZE_LIMIT) process.env.HANZO_FILE_SIZE_LIMIT = flags.HANZO_FILE_SIZE_LIMIT

        // Credentials
        if (flags.SECRETKEY_PATH) process.env.SECRETKEY_PATH = flags.SECRETKEY_PATH
        if (flags.HANZO_SECRETKEY_OVERWRITE) process.env.HANZO_SECRETKEY_OVERWRITE = flags.HANZO_SECRETKEY_OVERWRITE
        ...
        // Telemetry
        if (flags.DISABLE_HANZO_TELEMETRY) process.env.DISABLE_HANZO_TELEMETRY = flags.DISABLE_HANZO_TELEMETRY
        ...
        await (async () => {
            try {
                logger.info('Starting Hanzo...')
                await DataSource.init()
                await Server.start()
            } catch (error) {
                logger.error('There was an error starting Hanzo...', error)
