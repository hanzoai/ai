...
const deleteLoaderFromDocumentStore = async (storeId: string, loaderId: string) => {
    try {
        ...
        if (found) {
            ...
            throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Unable to locate loader in Document Store ${entity.name}`)
        }
    } catch (error) {
        throw new InternalHanzoError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const getDocumentStoreFileChunks = async (storeId: string, fileId: string, pageNo: number = 1) => {
    try {
        ...
        if (!chunksWithCount) {
            throw new InternalHanzoError(StatusCodes.NOT_FOUND, `File ${fileId} not found`)
        }
...
const deleteDocumentStoreFileChunk = async (storeId: string, docId: string, chunkId: string) => {
    try {
        ...
        if (!entity) {
            throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Document store ${storeId} not found`)
        }
        const loaders = JSON.parse(entity.loaders)
        const found = loaders.find((ldr: IDocumentStoreLoader) => ldr.id === docId)
        if (!found) {
            throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Document store loader ${docId} not found`)
        }
        ...
        if (!tbdChunk) {
            throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Document Chunk ${chunkId} not found`)
        }
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (input.startsWith('FILE-STORAGE::')) {
            if (!documentStoreEntity) {
                ...
                if (!documentStoreEntity) {
                    throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Document store ${data.storeId} not found`)
                }
            }
