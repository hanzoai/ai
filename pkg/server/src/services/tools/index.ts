...
const createTool = async (requestBody: any): Promise<any> => {
    try {
        ...
        throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.createTool - ${getErrorMessage(error)}`)
    }
}
...
const deleteTool = async (toolId: string): Promise<any> => {
    try {
        ...
        throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.deleteTool - ${getErrorMessage(error)}`)
    }
}
...
const getAllTools = async (): Promise<any> => {
    try {
        ...
        throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.getAllTools - ${getErrorMessage(error)}`)
    }
}
...
const getToolById = async (toolId: string): Promise<any> => {
    try {
        ...
        if (!dbResponse) {
            throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Tool ${toolId} not found`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.getToolById - ${getErrorMessage(error)}`)
    }
}
...
const updateTool = async (toolId: string, toolBody: any): Promise<any> => {
    try {
        ...
        throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.updateTool - ${getErrorMessage(error)}`)
    }
}
