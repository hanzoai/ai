...
const createAssistant = async (requestBody: any): Promise<any> => {
    try {
        ...
        if (!requestBody.details) {
            throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Invalid request body`)
        }
        ...
        try {
            ...
            if (!credential) {
                throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Credential ${requestBody.credential} not found`)
            }
            ...
            if (!openAIApiKey) {
                throw new InternalHanzoError(StatusCodes.NOT_FOUND, `OpenAI ApiKey not found`)
            }
            ...
            throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Error creating new assistant - ${getErrorMessage(error)}`)
        }
...
const deleteAssistant = async (assistantId: string, isDeleteBoth: any): Promise<any> => {
    try {
        ...
        if (!assistant) {
            throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Assistant ${assistantId} not found`)
        }
        try {
            ...
            if (!credential) {
                throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Credential ${assistant.credential} not found`)
            }
            ...
            if (error.status === 404 && error.type === 'invalid_request_error') {
                ...
                throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Error deleting assistant - ${getErrorMessage(error)}`)
            }
        }
    } catch (error) {
        throw new InternalHanzoError(
...
const updateAssistant = async (assistantId: string, requestBody: any): Promise<any> => {
    try {
        ...
        try {
            ...
            if (!credential) {
                throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Credential ${body.credential} not found`)
            }
            ...
            throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Error updating assistant - ${getErrorMessage(error)}`)
        }
    } catch (error) {
        throw new InternalHanzoError(
            StatusCodes.INTERNAL_SERVER_ERROR,
