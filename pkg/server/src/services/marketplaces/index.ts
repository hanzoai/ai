...
const getAllTemplates = async () => {
    try {
        ...
        const HanzoDocsQnAIndex = sortedTemplates.findIndex((tmp) => tmp.templateName === 'Hanzo Docs QnA')
        if (HanzoDocsQnAIndex > 0) {
            sortedTemplates.unshift(sortedTemplates.splice(HanzoDocsQnAIndex, 1)[0])
        }
