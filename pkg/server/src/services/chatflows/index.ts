import { removeFolderFromStorage } from '@hanzo/ai-components'
...
const checkIfChatflowIsValidForStreaming = async (chatflowId: string): Promise<any> => {
    try {
        ...
        if (!chatflow) {
            throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Chatflow ${chatflowId} not found`)
        }
...
const getChatflowByApiKey = async (apiKeyId: string, keyonly?: unknown): Promise<any> => {
    try {
        ...
        if (dbResponse.length < 1) {
            throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Chatflow not found in the database!`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalHanzoError(
...
const getChatflowById = async (chatflowId: string): Promise<any> => {
    try {
        ...
        if (!dbResponse) {
            throw new InternalHanzoError(StatusCodes.NOT_FOUND, `Chatflow ${chatflowId} not found in the database!`)
        }
        return dbResponse
    } catch (error) {
        throw new InternalHanzoError(
    ...
    try {
        ...
        if (dbResponse.chatbotConfig || uploadsConfig) {
            try {
                ...
                throw new InternalHanzoError(StatusCodes.INTERNAL_SERVER_ERROR, `Error parsing Chatbot Config for Chatflow ${chatflowId}`)
            }
        }
        return 'OK'
    } catch (error) {
