...
func main() {
	...
	//
	// routers.App is the fully-wrapped native router (assigned at package init,
	// filters installed by ai.Bootstrap above). The MsgType 200 gateway handler is
	// built around it, so every path the ad-hoc registry does not claim reaches the
	// same route table the :8000 surface serves — with its method and its path
	// parameters intact. It is an argument, not a follow-up call: without it the
	// gateway 404s everything and says nothing.
	object.InitZap()
