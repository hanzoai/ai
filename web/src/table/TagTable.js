...
class TagTable extends React.Component {
  ...
  renderTable(table) {
    ...
    const allKeys = (table || []).map((record, index) => `${index}`);
    const allSelected = allKeys.length > 0 && allKeys.every(key => this.state.selectedRowKeys.includes(key));
    const toggleRow = (key) => {
      this.setState((prevState) => ({
        selectedRowKeys: prevState.selectedRowKeys.includes(key)
          ? prevState.selectedRowKeys.filter(k => k !== key)
          : [...prevState.selectedRowKeys, key],
      }));
    };
    const toggleAll = () => {
      this.setState({selectedRowKeys: allSelected ? [] : allKeys});
    };

    return (
      <div>
        {this.props.title ? <div className="flex items-center flex-wrap gap-2 mb-2">{this.props.title}</div> : null}
        <div className="overflow-x-auto border border-zinc-800 rounded-lg"><table className="w-full text-sm text-left"><thead className="bg-zinc-900/80 border-b border-zinc-800"><tr><th className="px-3 py-2 w-10"><input type="checkbox" checked={allSelected} onChange={toggleAll} /></th>{columns.map(col => <th key={col.key || col.dataIndex} className="px-3 py-2 text-xs font-medium text-zinc-400 whitespace-nowrap">{col.title}</th>)}</tr></thead><tbody className="divide-y divide-zinc-800/50">{(table || []).map((record, index) => <tr key={record["id"] || index} className="hover:bg-zinc-900/50 transition-colors"><td className="px-3 py-2"><input type="checkbox" checked={this.state.selectedRowKeys.includes(`${index}`)} onChange={() => toggleRow(`${index}`)} /></td>{columns.map(col => <td key={col.key || col.dataIndex} className="px-3 py-2 text-zinc-300 whitespace-nowrap">{col.render ? col.render(record[col.dataIndex], record, index) : record[col.dataIndex]}</td>)}</tr>)}</tbody></table></div>
      </div>
    );
  }
