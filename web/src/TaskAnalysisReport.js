    ...
    <div style={{marginTop: "16px"}}>
      ...
      {categories.map((cat, idx) => (
        <div key={idx} style={{marginBottom: "24px"}}>
          ...
          <div className="overflow-x-auto border border-zinc-800 rounded-lg"><table className="w-full text-sm text-left"><thead className="bg-zinc-900/80 border-b border-zinc-800"><tr>{reportColumns.map(col => <th key={col.key || col.dataIndex} className="px-3 py-2 text-xs font-medium text-zinc-400 whitespace-nowrap">{col.title}</th>)}</tr></thead><tbody className="divide-y divide-zinc-800/50">{((cat.items || []).map((item, i) => ({...item, key: i}))).map((record, index) => <tr key={record.key !== undefined ? record.key : index} className="hover:bg-zinc-900/50 transition-colors">{reportColumns.map(col => <td key={col.key || col.dataIndex} className="px-3 py-2 text-zinc-300 whitespace-nowrap">{col.render ? col.render(record[col.dataIndex], record, index) : record[col.dataIndex]}</td>)}</tr>)}</tbody></table></div>
        </div>
      ))}
    </div>
  );
