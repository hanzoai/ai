...
func newEthereumClient(rpcURL, privateKeyHex, contractAddressHex, contractMethod string, lang string) (*EthereumClient, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to connect to Ethereum RPC: %v"), err))
	}
...
func (client *EthereumClient) Commit(data string, lang string) (string, string, string, error) {
	...
	if err != nil {
		return "", "", "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to get pending nonce: %v"), err))
	}
	gasPrice, err := client.Client.SuggestGasPrice(context.Background())
	if err != nil {
		return "", "", "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to suggest gas price: %v"), err))
	}
	value := big.NewInt(0)

	dataBytes, err := packFunctionData(client.ContractMethod, data, lang)
	if err != nil {
		return "", "", "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to pack function data: %v"), err))
	}
	...
	if err != nil {
		return "", "", "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to estimate gas: %v"), err))
	}
	...
	if err != nil {
		return "", "", "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to get chain ID: %v"), err))
	}
	signedTx, err := types.SignTx(tx, types.LatestSignerForChainID(chainID), client.PrivateKey)
	if err != nil {
		return "", "", "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to sign transaction: %v"), err))
	}
	err = client.Client.SendTransaction(context.Background(), signedTx)
	if err != nil {
		return "", "", "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to send transaction: %v"), err))
	}
	...
	if err != nil {
		return "", "", "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:transaction failed to be mined: %v"), err))
	}
	...
	if receipt.Status != types.ReceiptStatusSuccessful {
		...
		return "", "", "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:transaction failed with status: %d, reason: %s"), receipt.Status, revertReason))
	}
...
func (client *EthereumClient) Query(txHash string, data string, lang string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to get transaction receipt: %v"), err))
	}
	blockId := receipt.BlockNumber.String()

	stringType, err := abi.NewType("string", "", nil)
	if err != nil {
		return "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to create string type: %v"), err))
	}
	...
	for _, log := range receipt.Logs {
		if len(log.Data) > 0 {
			...
			if err != nil {
				return "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to unpack event log: %v"), err))
			}
			for _, v := range eventData {
				if str, ok := v.(string); ok {
					events = append(events, str)
				} else {
					return "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:event log value is not string: %v"), v))
				}
			}
		}
	}
	...
	if len(events) < 3 {
		return "", fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:not enough string events found in transaction logs, expected at least 3, got %d"), len(events)))
	}
...
func packFunctionData(method, data string, lang string) ([]byte, error) {
	...
	if err := json.Unmarshal([]byte(data), &uploadData); err != nil {
		return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to unmarshal JSON: %v"), err))
	}
	...
	if err != nil {
		return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to create tuple type: %v"), err))
	}
	...
	if err != nil {
		return nil, fmt.Errorf("%s", fmt.Sprintf(i18n.Translate(lang, "chain:failed to pack arguments: %v"), err))
	}
