...
func IsZapAvailable(clientId string) bool {
	...
	zapCommands := []string{"zap.bat", "zap-cli", "zap", "zap.bat"}
...
func NewZapScanProvider(clientId string) (*ZapScanProvider, error) {
	...
	if provider.zapPath == "" {
		...
		if provider.zapPath == "" {
			return nil, fmt.Errorf("%s ZAP not found in system PATH, please specify the path to ZAP binary (zap.bat, zap-cli, etc.)", getHostnamePrefix())
		}
	}
