...
func zapChatGraphAuthz(name string, user *iam.User) *zap.Message {
	...
	if !util.IsAdmin(user) {
		msg, _ := zapError(403, "this operation requires admin privilege")
		return msg
	}
	return nil
}
...
func zapIsCurrentUser(user *iam.User, input string) *zap.Message {
	...
	if !isName(username) || !isName(input) {
		msg, _ := zapError(403, "Unauthorized operation")
		return msg
	}
...
func zapGetChatHandler(_ context.Context, auth string, body []byte) (*zap.Message, error) {
	...
	// Which organization the chat belongs to. zapIsCurrentUser is not asked of an
	// admin, so this is what keeps an admin inside their own.
	...
	if !util.IsAdmin(user) && !conf.IsPreviewMode() {
		...
		if username != chat.User {
			return zapError(403, "Unauthorized operation")
		}
	}
	return zapOk(chat)
}
...
func zapAddChatHandler(_ context.Context, auth string, body []byte) (*zap.Message, error) {
	...
	if chat.Store == "" {
		...
		if store == nil {
			return zapError(200, "The default store is not found")
		}
...
func zapGetMessageHandler(_ context.Context, auth string, body []byte) (*zap.Message, error) {
	...
	if message == nil {
		return zapError(200, "Message not found")
	}
...
func zapUpdateMessageHandler(_ context.Context, auth string, body []byte) (*zap.Message, error) {
	...
	if storedMessage == nil {
		return zapError(200, "The message: "+id+" is not found")
	}
...
func zapAddMessageHandler(_ context.Context, auth string, body []byte) (*zap.Message, error) {
	...
	if message.Chat == "" {
		...
		if chat == nil {
			return zapError(200, "chat:The chat: "+chatId+" is not found")
		}
	}
	...
	if message.Text == "" {
		return zapError(200, "The question should not be empty")
	}
	...
	if store != nil {
		...
		if contains {
			return zapError(200, "Your message contains a forbidden word: \""+forbiddenWord+"\"")
		}
	}
...
func zapDeleteWelcomeMessageHandler(_ context.Context, auth string, body []byte) (*zap.Message, error) {
	...
	if username == "" || username != message.User {
		return zapError(200, "No permission")
	}
