...
// TestZapWiringNoNode asserts both node-registration entry points are safe
// no-ops when ZAP is off (object.GetZapNode() == nil) — neither may panic and
// ai's HTTP behavior is unchanged. ZAP_ENABLED is unset in the test
// environment, so the node is never created.
