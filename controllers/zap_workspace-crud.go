...
func zapTemplatesHandler(ctx context.Context, method, path, query, auth string, body []byte) (*zap.Message, error) {
	name := zapControllerName(path)
	user := zapPrincipalUser(auth)
	if refuse := zapWorkspaceGate(user, name); refuse != nil {
		return refuse, nil
	}
	q, _ := url.ParseQuery(query)

	switch {
	case strings.HasPrefix(path, "/v1/get-templates"):
		owner, ok, refuse := zapScopedOwner(user, q)
		if !ok {
			return refuse, nil
		}
		return zapGetTemplates(owner, q)
	case strings.HasPrefix(path, "/v1/get-template"):
		res, err := object.GetTemplate(q.Get("id"))
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(res)
	case strings.HasPrefix(path, "/v1/update-template"):
		var t object.Template
		if err := json.Unmarshal(body, &t); err != nil {
			return zapGwError(200, err.Error())
		}
		success, err := object.UpdateTemplate(q.Get("id"), &t)
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(success)
	case strings.HasPrefix(path, "/v1/add-template"):
		var t object.Template
		if err := json.Unmarshal(body, &t); err != nil {
			return zapGwError(200, err.Error())
		}
		success, err := object.AddTemplate(&t)
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(success)
	case strings.HasPrefix(path, "/v1/delete-template"):
		var t object.Template
		if err := json.Unmarshal(body, &t); err != nil {
			return zapGwError(200, err.Error())
		}
		success, err := object.DeleteTemplate(&t)
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(success)
...
func zapWorkflowsHandler(ctx context.Context, method, path, query, auth string, body []byte) (*zap.Message, error) {
	name := zapControllerName(path)
	user := zapPrincipalUser(auth)
	if refuse := zapWorkspaceGate(user, name); refuse != nil {
		return refuse, nil
	...
	switch {
	case strings.HasPrefix(path, "/v1/get-global-workflows"):
		workflows, err := object.GetGlobalWorkflows()
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(object.GetMaskedWorkflows(workflows, true))
	case strings.HasPrefix(path, "/v1/get-workflows"):
		owner, ok, refuse := zapScopedOwner(user, q)
		if !ok {
			return refuse, nil
		}
		return zapGetWorkflows(owner, q)
	case strings.HasPrefix(path, "/v1/get-workflow"):
		workflow, err := object.GetWorkflow(q.Get("id"))
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(object.GetMaskedWorkflow(workflow, true))
	case strings.HasPrefix(path, "/v1/update-workflow"):
		var w object.Workflow
		if err := json.Unmarshal(body, &w); err != nil {
			return zapGwError(200, err.Error())
		}
		success, err := object.UpdateWorkflow(q.Get("id"), &w, "en")
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(success)
	case strings.HasPrefix(path, "/v1/add-workflow"):
		var w object.Workflow
		if err := json.Unmarshal(body, &w); err != nil {
			return zapGwError(200, err.Error())
		}
		success, err := object.AddWorkflow(&w, "en")
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(success)
	case strings.HasPrefix(path, "/v1/delete-workflow"):
		var w object.Workflow
		if err := json.Unmarshal(body, &w); err != nil {
			return zapGwError(200, err.Error())
		}
		success, err := object.DeleteWorkflow(&w)
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(success)
...
// ── Tasks (task.go + task_upload.go) ───────────────────────────────────────────

func zapTasksHandler(ctx context.Context, method, path, query, auth string, body []byte) (*zap.Message, error) {
	name := zapControllerName(path)
	user := zapPrincipalUser(auth)
	if refuse := zapWorkspaceGate(user, name); refuse != nil {
		return refuse, nil
	...
	isPreview := conf.IsPreviewMode()
	username := zapSessionUsername(user)

	switch {
	case strings.HasPrefix(path, "/v1/get-global-tasks"):
		owner := username
		if isAdmin {
			owner = ""
		}
	...
	case strings.HasPrefix(path, "/v1/get-tasks"):
		owner, ok, refuse := zapScopedOwner(user, q)
		if !ok {
			return refuse, nil
		}
		...
		return zapGetTasks(owner, q)

	case strings.HasPrefix(path, "/v1/get-task"):
		task, err := object.GetTask(q.Get("id"))
		if err != nil {
			return zapGwError(200, err.Error())
		}
		if task == nil {
		...
		if !isAdmin && !isPreview && task.Owner != username {
			return zapGwError(200, "auth:Unauthorized operation")
		}
		return zapGwOk(task)

	case strings.HasPrefix(path, "/v1/update-task"):
		id := q.Get("id")
		var task object.Task
		if err := json.Unmarshal(body, &task); err != nil {
			return zapGwError(200, err.Error())
		}
		existing, err := object.GetTask(id)
		if err != nil {
			return zapGwError(200, err.Error())
		}
		if existing == nil {
		...
		if !isAdmin && !isPreview && existing.Owner != username {
			return zapGwError(200, "auth:Unauthorized operation")
		}
	...
	case strings.HasPrefix(path, "/v1/add-task"):
		var task object.Task
		if err := json.Unmarshal(body, &task); err != nil {
			return zapGwError(200, err.Error())
		}
	...
	case strings.HasPrefix(path, "/v1/delete-task"):
		var task object.Task
		if err := json.Unmarshal(body, &task); err != nil {
			return zapGwError(200, err.Error())
		}
		if !isAdmin {
			existing, err := object.GetTask(task.GetId())
			if err != nil {
				return zapGwError(200, err.Error())
			}
			if existing == nil {
	...
	case strings.HasPrefix(path, "/v1/upload-task-document"):
		return zapUploadTaskDocument(user, q, body)

	case strings.HasPrefix(path, "/v1/analyze-task"):
		return zapAnalyzeTask(user, q)
	}
	return zapGwError(404, "not found: "+path)
}
...
func zapVideosCrudHandler(ctx context.Context, method, path, query, auth string, body []byte) (*zap.Message, error) {
	name := zapControllerName(path)
	user := zapPrincipalUser(auth)
	if refuse := zapWorkspaceGate(user, name); refuse != nil {
		return refuse, nil
	...
	switch {
	case strings.HasPrefix(path, "/v1/get-global-videos"):
	...
	case strings.HasPrefix(path, "/v1/get-videos"):
		if _, ok, refuse := zapScopedOwner(user, q); !ok {
			return refuse, nil
		}
		return zapGetVideos(q)

	case strings.HasPrefix(path, "/v1/get-video"):
		v, err := object.GetVideo(q.Get("id"), "en")
		if err != nil {
			return zapGwError(200, err.Error())
		}
		if v != nil {
			if err = v.Populate("en"); err != nil {
				return zapGwError(200, err.Error())
			}
		}
		return zapGwOk(v)

	case strings.HasPrefix(path, "/v1/update-video"):
		if user == nil {
			return zapGwError(401, "auth:Please sign in first")
		}
		var v object.Video
		...
		if util.IsVideoNormalUser(user) {
			if len(v.Remarks) > 0 || len(v.Remarks2) > 0 || v.State != "Draft" {
				return zapGwError(200, "video:The video can only be updated when there are no remarks and the state is \"Draft\"")
			}
		}
		success, err := object.UpdateVideo(q.Get("id"), &v)
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(success)

	case strings.HasPrefix(path, "/v1/add-video"):
		var v object.Video
		if err := json.Unmarshal(body, &v); err != nil {
			return zapGwError(200, err.Error())
		}
		success, err := object.AddVideo(&v)
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(success)

	case strings.HasPrefix(path, "/v1/delete-video"):
		var v object.Video
		if err := json.Unmarshal(body, &v); err != nil {
			return zapGwError(200, err.Error())
		}
		success, err := object.DeleteVideo(&v)
		if err != nil {
			return zapGwError(200, err.Error())
		}
		return zapGwOk(success)

	case strings.HasPrefix(path, "/v1/upload-video"):
		return zapUploadVideo(user, body)
	}
	return zapGwError(404, "not found: "+path)
}
