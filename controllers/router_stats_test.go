...
// The public platform scope must NEVER carry absolute $ levels or org identity, but
// SHOULD carry the safe ratio AND the REAL model ids (owner product decision: the
// world.hanzo.ai widget shows the true roster so users see which models win).
func TestComputeRouterStats_PublicScopeRedaction(t *testing.T) {
	...
	// Real ids now surface on the platform scope — the baseline + by_model carry the
	// true model ids (opus/mini), never opaque arm-N labels.
	if s.Cost.SavedPct <= 0 || s.Cost.BaselineModel != "opus" {
		t.Fatalf("platform scope shows the REAL baseline id opus, got %+v", s.Cost)
	}
	if s.ByModel["opus"] != 1 || s.ByModel["mini"] != 1 {
		t.Fatalf("expected real ids opus=1, mini=1, got %+v", s.ByModel)
	}
	for id := range s.ByModel {
		if id == "arm-1" || id == "arm-2" || id == "arm-other" {
			t.Fatalf("platform by_model must NOT carry arm-N labels anymore: %+v", s.ByModel)
		}
	}
	if s.ByTask["reasoning"].Models["opus"] != 1 {
		t.Fatalf("platform by_task must show the real id opus, got %+v", s.ByTask["reasoning"])
	}
