...
func TestUnpricedSpanAttr(t *testing.T) {
	...
	m := attrMap(buildGenAISpanFields(flagged, 0, 0, new(float64(0)), new(float64(0)), nil, false).attrs)
	...
	m = attrMap(buildGenAISpanFields(priced, 0, 0, new(float64(0)), new(float64(0)), nil, false).attrs)
