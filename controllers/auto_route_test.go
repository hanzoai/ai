...
// 404). enso-flash is a gated (waitlist) family SKU whose route resolves; glm-5.2 is a
// plain servable fallback; code prompts prefer enso-flash then glm-5.2. Without a grant
// the router skips enso-flash and falls through to glm-5.2; with a grant it is chosen.
// The gated status is real (discovered catalog); only the grant lookup — the DB leaf —
// is seamed, so this exercises the true grantAllows composition end-to-end.
