...
// migration of node.go / machine.go / docker_container.go / kubernetes_pod.go /
// image.go / vm.go / the request-response leg of tunnel.go off beego.
//
...
func registerZapInfraK8s() {
	pairs := []struct {
		...
		{"get-machines", zapGetMachinesHandler},
		{"get-machine", zapGetMachineHandler},
		{"add-machine", zapAddMachineHandler},
		{"update-machine", zapUpdateMachineHandler},
		{"delete-machine", zapDeleteMachineHandler},

		{"get-containers", zapGetContainersHandler},
		{"get-container", zapGetContainerHandler},
		{"add-container", zapAddContainerHandler},
		{"update-container", zapUpdateContainerHandler},
		{"delete-container", zapDeleteContainerHandler},

		{"get-pods", zapGetPodsHandler},
		{"get-pod", zapGetPodHandler},
		{"add-pod", zapAddPodHandler},
		{"update-pod", zapUpdatePodHandler},
		{"delete-pod", zapDeletePodHandler},

		{"get-images", zapGetImagesHandler},
		{"get-image", zapGetImageHandler},
		{"add-image", zapAddImageHandler},
		{"update-image", zapUpdateImageHandler},
		{"delete-image", zapDeleteImageHandler},
...
func zapGetMachinesHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	owner, _, reject := zapInfraSuperAdmin(auth)
	if reject != nil {
		return reject, nil
	}
	...
	if _, err := object.SyncMachinesCloud(owner, "en"); err != nil {
		return zapErr(500, err.Error())
	}
	if offset, limit, ok := p.paged(); ok {
		count, err := object.GetMachineCount(owner, p.Field, p.Value)
		if err != nil {
			return zapErr(500, err.Error())
		}
		machines, err := object.GetMaskedMachines(object.GetPaginationMachines(owner, offset, limit, p.Field, p.Value, p.SortField, p.SortOrder))
		if err != nil {
			return zapErr(500, err.Error())
		}
		return zapOk2(machines, count)
	}
	machines, err := object.GetMaskedMachines(object.GetMachines(owner))
	if err != nil {
		return zapErr(500, err.Error())
	}
	return zapOk(machines)
}

func zapGetMachineHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	id := infraId(body)
	...
	return zapOk(machine)
}

func zapAddMachineHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	...
	return zapActionOk(object.AddMachine(&machine))
}

func zapUpdateMachineHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	var machine object.Machine
	...
	return zapActionOk(object.UpdateMachine(infraId(body), &machine))
}

func zapDeleteMachineHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	var machine object.Machine
	...
	return zapActionOk(object.DeleteMachine(&machine))
}

// ── Containers ───────────────────────────────────────────────────────────────

func zapGetContainersHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	owner, _, reject := zapInfraSuperAdmin(auth)
	if reject != nil {
		return reject, nil
	}
	...
	if _, err := object.SyncDockerContainers(owner); err != nil {
		return zapErr(500, err.Error())
	}
	if offset, limit, ok := p.paged(); ok {
		count, err := object.GetContainerCount(owner, p.Field, p.Value)
		if err != nil {
			return zapErr(500, err.Error())
		}
		containers, err := object.GetMaskedContainers(object.GetPaginationContainers(owner, offset, limit, p.Field, p.Value, p.SortField, p.SortOrder))
		if err != nil {
			return zapErr(500, err.Error())
		}
		return zapOk2(containers, count)
	}
	containers, err := object.GetMaskedContainers(object.GetContainers(owner))
	if err != nil {
		return zapErr(500, err.Error())
	}
	return zapOk(containers)
}

func zapGetContainerHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	id := infraId(body)
	...
	return zapOk(container)
}

func zapAddContainerHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	...
	return zapActionOk(object.AddContainer(&container))
}

func zapUpdateContainerHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	var container object.Container
	...
	return zapActionOk(object.UpdateContainer(infraId(body), &container, "en"))
}

func zapDeleteContainerHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	var container object.Container
	...
	return zapActionOk(object.DeleteContainer(&container))
}

// ── Pods ─────────────────────────────────────────────────────────────────────

func zapGetPodsHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	owner, _, reject := zapInfraSuperAdmin(auth)
	if reject != nil {
		return reject, nil
	}
	...
	if _, err := object.SyncKubernetesPods(owner); err != nil {
		return zapErr(500, err.Error())
	}
	if offset, limit, ok := p.paged(); ok {
		count, err := object.GetPodCount(owner, p.Field, p.Value)
		if err != nil {
			return zapErr(500, err.Error())
		}
		pods, err := object.GetMaskedPods(object.GetPaginationPods(owner, offset, limit, p.Field, p.Value, p.SortField, p.SortOrder))
		if err != nil {
			return zapErr(500, err.Error())
		}
		return zapOk2(pods, count)
	}
...
func zapGetPodHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	id := infraId(body)
...
func zapAddPodHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	var pod object.Pod
	...
	return zapActionOk(object.AddPod(&pod))
}

func zapUpdatePodHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	var pod object.Pod
	...
	return zapActionOk(object.UpdatePod(infraId(body), &pod, "en"))
}

func zapDeletePodHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	var pod object.Pod
	...
	return zapActionOk(object.DeletePod(&pod))
}

// ── Images ───────────────────────────────────────────────────────────────────

func zapGetImagesHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	owner, _, reject := zapInfraSuperAdmin(auth)
	if reject != nil {
		return reject, nil
	}
	...
	if _, err := object.SyncImagesCloud(owner, "en"); err != nil {
		return zapErr(500, err.Error())
	}
	if offset, limit, ok := p.paged(); ok {
		count, err := object.GetImageCount(owner, p.Field, p.Value)
		if err != nil {
			return zapErr(500, err.Error())
		}
		images, err := object.GetMaskedImages(object.GetPaginationImages(owner, offset, limit, p.Field, p.Value, p.SortField, p.SortOrder))
		if err != nil {
			return zapErr(500, err.Error())
		}
		return zapOk2(images, count)
	}
...
func zapGetImageHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	image, err := object.GetMaskedImage(object.GetImage(infraId(body)))
	if err != nil {
		return zapErr(500, err.Error())
	}
	return zapOk(image)
...
func zapAddImageHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	...
	return zapActionResponse(object.AddImage(&image))
}

func zapUpdateImageHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	var image object.Image
	...
	return zapActionResponse(object.UpdateImage(infraId(body), &image))
}

func zapDeleteImageHandler(ctx context.Context, auth string, body []byte) (*zap.Message, error) {
	if _, _, reject := zapInfraSuperAdmin(auth); reject != nil {
		return reject, nil
	}
	var image object.Image
	...
	return zapActionResponse(object.DeleteImage(&image))
}
