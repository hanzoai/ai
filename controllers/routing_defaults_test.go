...
func TestRoutingEventFiredOnResolve(t *testing.T) {
	...
	model, _, ok := resolveAutoModel("auto", "acme", "acme/alice", "req-fired",
		nil, chatReq("auto", "please refactor this function"), router.Slo{})
	if !ok || model != "glm-5.2" {
...
func TestRoutingEventAbsentOnNonAuto(t *testing.T) {
	...
	globalModelConfig = routerTestConfig(true)
	if _, _, ok := resolveAutoModel("gpt-4o", "acme", "acme/alice", "", nil, chatReq("gpt-4o", "hi"), router.Slo{}); ok {
		t.Fatal("resolveAutoModel routed a concrete model")
	}

	globalModelConfig = routerTestConfig(false) // routing globally off
	if _, _, ok := resolveAutoModel("auto", "acme", "acme/alice", "", nil, chatReq("auto", "hi"), router.Slo{}); ok {
