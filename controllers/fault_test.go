...
// serving nobody.
func proxied(t *testing.T, status int, body string) (c *ApiController, asked bool) {
	t.Helper()
...
func TestAProxiedVendorRefusalIsNotTheCallersBill(t *testing.T) {
	c, asked := proxied(t, http.StatusPaymentRequired, `{"error":{"message":"Insufficient credits."}}`)
	if !asked {
	...
	if answered(c) != http.StatusServiceUnavailable {
		t.Errorf("status=%d, want 503 — the true statement is that we could not serve this", answered(c))
	}
	if got := codeOf(exhausted("glm-5.2", nil)); !strings.Contains(sent(c), got) {
...
func TestAFrontedBillingNoticeStillReachesTheCaller(t *testing.T) {
	...
	c, asked := proxied(t, http.StatusPaymentRequired, string(notice.ErrorJSON()))
	if !asked {
		t.Fatal("the upstream was never dialled")
	}
	if answered(c) != http.StatusPaymentRequired {
		t.Errorf("status=%d, want 402 — this refusal is our own gate speaking through a "+
			"service that fronts for us, and the debt really is the caller's", answered(c))
	}
}
...
func scrape(t *testing.T) string {
	...
	object.MetricsHandler().ServeHTTP(exposition, httptest.NewRequest(http.MethodGet, "/v1/metrics", nil))
	if exposition.Code != http.StatusOK {
		t.Fatalf("metrics scrape = %d, want 200", exposition.Code)
	}
	return exposition.Body.String()
}
...
func TestAProxiedRequestFaultKeepsItsStatus(t *testing.T) {
	c, asked := proxied(t, http.StatusBadRequest, `{"error":{"message":"invalid 'messages': empty"}}`)
	if !asked {
		t.Fatal("the upstream was never dialled")
	}
