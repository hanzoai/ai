...
// TestImagesGenerationsRouteIsRegistered proves POST /v1/images/generations is
// wired to the real ImagesGenerations controller through the unified binary's
...
func TestImagesGenerationsRouteIsRegistered(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "http://example.com/v1/images/generations",
		strings.NewReader(`{"model":"zen3-image","prompt":"a cat"}`))
	...
	if resp.StatusCode == http.StatusNotFound {
		t.Fatalf("POST /v1/images/generations: 404 — route not registered (the bug). body=%q", body)
	}
...
// ensure the controllers package is linked (ImagesGenerations method exists).
var _ = (&controllers.ApiController{}).ImagesGenerations
